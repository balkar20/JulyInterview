...
namespace Incapsulation.Failures
{
    ...
    public class ReportMaker
    {
            ...
            List<Dictionary<string, object>> devices)
        {
            ...
            List<Device> devicesList = ConverDictionariesToDevices(devices);
            List<Failure> failures = new List<Failure>();
            for (int i = 0; i < failureTypes.Length; i++)
            {
                failures.Add(new Failure()
                {
                    FailureType = Common.IsFailureSerious(failureTypes[i]) == 1 ? FailureType.Serious : FailureType.NotSerious,
            ...
            List<Device> devices)
        {
            ...
            var result = new List<string>();
            foreach (var device in devices)
                if (problematicDevices.Contains((int)device.Id))
                    result.Add(device.Name as string);

            return result;
        }

        private static List<Device> ConverDictionariesToDevices(List<Dictionary<string, object>> dictionaries)
        {
